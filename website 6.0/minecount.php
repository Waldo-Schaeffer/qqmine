...
function html_header () {
    ...
    echo            "
					...
					今天爆出的礼物统计，数据仅供参考，具体以实际情况为准。（没有办法统计小礼物的情况）
...
function html_center () {
    ...
    # echo $table_name;
    $sql = 'SELECT count(*) FROM ' . $table_name .' where (gift_name like "%独角兽%") and from_unixtime(`gift_time` /1000) >= date(now()) ';
	...
	$gift_name = '夺宝战机';
	$gift_value = -5000;
	$sql = 'SELECT count(*) FROM ' . $table_name .' where (gift_name like "%' . $gift_name . '%") and from_unixtime(`gift_time` /1000) >= date(now()) ';
    $query_result = mysqli_query($handle, $sql);
