...
function html_center () {
	...
	# echo $table_name;
    $sql = 'select date,nick,name,num from ' . $table_name . ' where (name <=> "神碎片"  or name <=> "黄金大炮" or name <=> "黄金战机" or name <=> "3星衰神卡" or name <=> "2星衰神卡" or name <=> "财神卡" or name <=>   "高级挖矿券") order by date desc limit 0,550';
    $query_result = mysqli_query($handle, $sql);
    if (!$query_result) {
        printf("Error: %s\n", mysqli_error($handle));
        exit();
