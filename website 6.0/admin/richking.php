...
function html_center () {
	...
	# echo $table_name;
    $sql = 'select date,nick,name,num from ' . $table_name . ' where (name <=> "皇家招财猫"  or name <=> "皇家钞票枪" or name <=> "皇家同花顺" or name <=> "风铃禾梦" or name <=> "盛宴黑桃A" or name <=> "无间道" or name <=> "甜心宝蓓") order by date desc limit 0,550';
	$query_result = mysqli_query($handle, $sql);
    if (!$query_result) {
        printf("Error: %s\n", mysqli_error($handle));
        exit();
