...
function html_center () {
	...
	# echo $table_name;
    $sql = 'select date,nick,name,num from ' . $table_name . ' where (name <=> "独角兽（30天）") order by date desc limit 0,20';
	if (isset($_SESSION['nickname'])) {
		$sql = 'select date,nick,name,num from ' . $table_name . ' where (name <=> "独角兽（30天）") order by date desc limit 0,1050';
	}
    $query_result = mysqli_query($handle, $sql);
    if (!$query_result) {
        printf("Error: %s\n", mysqli_error($handle));
