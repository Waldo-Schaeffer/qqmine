...
def db_connect():
    ...
    # 创建数据库表：
    sql_sub_table_magic = """CREATE TABLE IF NOT EXISTS `sub_table_magic` (
    ...
    sql_data_table = """CREATE TABLE IF NOT EXISTS `magic_1` (
    ...
    cursor.execute(sql_sub_table_magic)
    cursor.execute(sql_data_table)

    # 初始化 sub_table 表数据,表为空才插入
    sql_test = "select * from sub_table_magic"
    sql_init_sub_table = "insert into sub_table_magic(table_name) values('magic_1');"
...
def db_operation(gift_data, temp_str):
    ...
    cursor.execute('select max(table_id) from sub_table_magic;')
    table_id = cursor.fetchall()[0][0]     # sub_table表中的table_id
    table_name = 'magic_' + str(table_id)
    ...
    if not temp_fetchall or str(gift_data[0]) > temp_fetchall :     # 首先判断数据是否是空的，防止第一次建立数据库时出错
        ...
        # 如果表中数据达到4万，则创建分表
        if key_id > 40000 :
            sql_data_table = """CREATE TABLE IF NOT EXISTS `magic_""" + str(table_id+1) + """` (
                    `key_id` INT NOT NULL AUTO_INCREMENT,
                    `create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP comment '创建时间',
                    `update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
                    `gift_time` VARCHAR(255) NOT NULL,
            ...
            try:
                ...
                cursor.execute("insert into sub_table_magic(table_id, table_name) values(" + str(table_id+1) + ", 'magic_" + str(table_id+1) + "');")
                conn.commit()
                cursor.execute("insert into magic_" + str(table_id+1) + " ( `create_time`, `update_time`, `gift_time`, `gift_author`, `gift_name`, `gift_number`, `gift_color`, `gift_master`) select `create_time`, `update_time`, `gift_time`, `gift_author`, `gift_name`, `gift_number`, `gift_color`, `gift_master` from magic_" + str(table_id) + " where key_id >= (40000 - 500 - 1) order by gift_time")
                conn.commit()
            except Exception as e:
                print(e)
                with open (log_name, 'a') as log_handle:
