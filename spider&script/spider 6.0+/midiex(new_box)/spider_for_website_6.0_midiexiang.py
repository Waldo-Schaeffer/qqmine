...
def db_connect():
    ...
    # 创建数据库表：
    sql_sub_table_midiex = """CREATE TABLE IF NOT EXISTS `sub_table_midiex` (
    ...
    sql_data_table = """CREATE TABLE IF NOT EXISTS `midiex_1` (
            ...
            `gift_box` VARCHAR(255) NOT NULL,
            `gift_boxtype` VARCHAR(255) NOT NULL,
            primary key(key_id)
            )ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;"""
    cursor.execute(sql_sub_table_midiex)
    cursor.execute(sql_data_table)

    # 初始化 sub_table 表数据,表为空才插入
    sql_test = "select * from sub_table_midiex"
    sql_init_sub_table = "insert into sub_table_midiex(table_name) values('midiex_1');"
...
def db_operation(gift_data, temp_str):
    ...
    cursor.execute('select max(table_id) from sub_table_midiex;')
    table_id = cursor.fetchall()[0][0]     # sub_table表中的table_id
    table_name = 'midiex_' + str(table_id)
    ...
    if not temp_fetchall or str(gift_data[0]) > temp_fetchall :     # 首先判断数据是否是空的，防止第一次建立数据库时出错
        ...
        sql_insert_data = "insert into " + table_name + "(gift_time,gift_author,gift_name,gift_number,gift_color,gift_master,gift_box,gift_boxtype) values('"+str(gift_data[0])+"','"+gift_data[1]+"','"+gift_data[2]+"',"+str(gift_data[3])+",'"+gift_data[4]+"','"+gift_data[5]+"','"+gift_data[6]+"','"+gift_data[7]+"');"
        try:
        ...
        # 如果表中数据达到4万，则创建分表
        if key_id > 40000 :
            sql_data_table = """CREATE TABLE IF NOT EXISTS `midiex_""" + str(table_id+1) + """` (
                    `key_id` INT NOT NULL AUTO_INCREMENT,
                    `create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP comment '创建时间',
                    `update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
                    `gift_time` VARCHAR(255) NOT NULL,
            ...
            try:
                ...
                cursor.execute("insert into sub_table_midiex(table_id, table_name) values(" + str(table_id+1) + ", 'midiex_" + str(table_id+1) + "');")
                conn.commit()
                cursor.execute("insert into midiex_" + str(table_id+1) + " ( `create_time`, `update_time`, `gift_time`, `gift_author`, `gift_name`, `gift_number`, `gift_color`, `gift_master`, `gift_box`,`gift_boxtype`) select `create_time`, `update_time`, `gift_time`, `gift_author`, `gift_name`, `gift_number`, `gift_color`, `gift_master`, `gift_box`, `gift_boxtype` from midiex_" + str(table_id) + " where key_id >= (40000 - 500 - 1) order by gift_time")
                conn.commit()
            except Exception as e:
                print(e)
                with open (log_name, 'a') as log_handle:
        ...
        if msg :
            ...
            for msg_list in new_msg :
                ...
                try:
                    ...
                    gift_data.append(re_data[0][3])                 # [2] name
                    gift_data.append(re_data[0][4])                 # [3] number
                    gift_data.append(msg_list['ext']['0_c'])        # [4] color
                    if re_data[0][0] == '':                         # 空直播间直接忽略
                        continue
                    if ' ' in re_data[0][0] :
                        gift_data.append(re_data[0][0].split('在 ')[1])
                    else:
                        gift_data.append(re_data[0][0])             # [5] master
                    gift_data.append(re_data[0][1])                 # [6] box
                    gift_data.append(re_data[0][2])                 # [6] boxtype
                except:
