...
def db_connect():
    ...
    # 创建数据库表：
    sql_sub_table = """CREATE TABLE IF NOT EXISTS `sub_table_emerald` (
    ...
    sql_data_table = """CREATE TABLE IF NOT EXISTS `qqegame_gift_emerald_1` (
    ...
    sql_test = "select * from sub_table_emerald"
    sql_init_sub_table = "insert into sub_table_emerald(table_name) values('qqegame_gift_emerald_1');"
...
def db_operation(qqegame_gift, temp_str):
    ...
    cursor.execute('select max(table_id) from sub_table_emerald;')
    table_id = cursor.fetchall()[0][0]     # sub_table表中的table_id
    table_name = 'qqegame_gift_emerald_' + str(table_id)
    ...
    if not temp_fetchall or str(qqegame_gift[3]) >= str(temp_fetchall) :     # 首先判断数据是否是空的，防止第一次建立数据库时出错
        
        # 如果同一时间出多个礼物，则查询是否已插入过该条数据
        if str(qqegame_gift[3]) == str(temp_fetchall) :
            cursor.execute('select 1 from '+table_name+' where source = "'+qqegame_gift[4]+'" limit 1 ;' )
            if cursor.fetchall():
                # print ('null')
                return;
        # print (qqegame_gift[4])
        ...
        # 如果表中数据达到4万，则创建分表
        if key_id > 20000 :
            sql_data_table = """CREATE TABLE IF NOT EXISTS `qqegame_gift_emerald_""" + str(table_id+1) + """` (
                    `key_id` INT NOT NULL AUTO_INCREMENT,
                    `create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP comment '创建时间',
                    `update_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
                    `name` VARCHAR(255) NOT NULL,
            ...
            try:
                ...
                cursor.execute("insert into sub_table_emerald(table_id, table_name) values(" + str(table_id+1) + ", 'qqegame_gift_emerald_" + str(table_id+1) + "');")
                conn.commit()
                cursor.execute("insert into qqegame_gift_emerald_" + str(table_id+1) + " ( `create_time`, `update_time`, `name`, `num`, `nick`, `date`, `source`) select `create_time`, `update_time`, `name`, `num`, `nick`, `date`, `source` from qqegame_gift_" + str(table_id) + " where key_id >= (20000 - 500 - 1) order by date")
                conn.commit()
            except Exception as e:
                print(e)
                with open (log_name, 'a') as log_handle:
...
def main():
    ...
    while 1:
        ...
        while 1:
            ...
            if number_first == 1 :
                data = {
                    'param':'{"0":{"param":{"type":1,"anchor_id":0,"name":"","ts":' + str(nowTime / 1000) + ',"page_num":1,"page_size":50},"module":"pgg_loot_boxes_mt_svr","method":"get_loot_prize_detail"}}'
                }
                number_first = 0
            else:
                data = {
                    'param':'{"0":{"param":{"type":1,"anchor_id":0,"name":"","ts":' + str(nowTime / 1000) + ',"page_num":1,"page_size":15},"module":"pgg_loot_boxes_mt_svr","method":"get_loot_prize_detail"}}'
                }
                time.sleep(1)
                break_time += 1
            try:
            ...
            if msg :
                ...
                for msg_list in new_msg :
                    ...
                    try:
                        ...
                        if (time.strftime("%H:%M:%S", time.localtime()) < '01:00:00' and msg_list['date'] > '20:00:00') :
                            log_handle.write('自动纠正日期问题')
                            jintian = ((datetime.date.today() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d")).strftime("%Y-%m-%d")
                    ...
                    if len(qqegame_gift) == 5:
                        # print(qqegame_gift[3]+'===>>'+qqegame_gift[4])
